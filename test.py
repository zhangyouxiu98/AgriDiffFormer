    ...
    def test_phase():
        ...
        with torch.no_grad():
            ...
            for imgs_A, imgs_B, labels_A, labels_B, _, _, name in loop:
                ...
                io.imsave(args.labelA_rgb_path + "/" + ''.join(name)+ ".png", pred_A_color)
                pred_B_color = RS.Index2Color(preds_B.squeeze().to("cpu").numpy())
                io.imsave(args.labelB_rgb_path + "/" + ''.join(name)+ ".png", pred_B_color)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Parameter for dat analysis, data cleaning and model training.")
    parser.add_argument("--dataname", default="Landsat", type=str, help="data name")
    parser.add_argument("--modelname", default="diff+ASD", type=str, help="model name")
    parser.add_argument("--datapath", default=r"E:\zjl\SCD\dataset\Landsat_BT", type=str,
                        help="data path")
    parser.add_argument("--ckptpath", default="checkpoints/Landsat/DiffFormer_ABD/run_0000/E47_mIoU80.3_Sek36.1.pth", type=str,
                        help="ckpt path")
    parser.add_argument("--vispath", default="results_Landsat_diff_ASD/", type=str,
                        help="vis path")
